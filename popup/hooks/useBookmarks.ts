			...
			const collectFolders = (nodes: any[], path = ""): BookmarkFolder[] => {
				...
				for (const node of nodes) {
					if (node.parentId === "0") {
						// 直接处理其子节点，不将当前节点加入路径
						if (node.children) {
							const childFolders = collectFolders(node.children, "");
	...
	const createFolder = async (folderPath: string): Promise<string> => {
		try {
			...
			const getBookmarkTree = (): Promise<any[]> => {
				return new Promise((resolve, reject) => {
					if (typeof chrome !== "undefined") {
						...
						browser.bookmarks.getTree().then(resolve).catch(reject);
					} else {
			...
			const createBookmarkFolder = (
				...
				return new Promise((resolve, reject) => {
					if (typeof chrome !== "undefined") {
						chrome.bookmarks.create({ parentId, title }, (folder: any) =>
							resolve(folder.id),
						);
					} else if (typeof browser !== "undefined") {
						browser.bookmarks
			...
			const findLastLevelFolder = (
				...
				for (const node of tree) {
					if (node.children) {
						...
						// 递归查找子节点
						const result = findLastLevelFolder(node.children, folderName);
						if (result) return result;
					}
				}
				return null;
			...
			for (let i = 0; i < pathParts.length; i++) {
				...
				if (existingFolderId) {
					...
					// 如果不存在，先尝试查找同名的最后一级文件夹
					const lastLevelFolderId = findLastLevelFolder(tree, pathParts[i]);
					if (lastLevelFolderId) {
						// 如果找到同名文件夹，使用它作为父文件夹
						parentId = lastLevelFolderId;
					} else {
						// 如果没有找到同名文件夹，则在当前 parentId 下创建新文件夹
